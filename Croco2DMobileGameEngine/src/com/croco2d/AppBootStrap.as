package com.croco2d
{
	...
	public class AppBootStrap extends Sprite
	{
		...
		{
			const globalParamsConfig:Object = AppConfig.globalParamsConfig;
			if(globalParamsConfig.pauseWhenDeActivated)
			{
				if(__crocoEngine) __crocoEngine.stop();
				if(__starling) __starling.stop();
			}
		...
		protected function appActivateHandler(event:*):void
		{
			...
			if(globalParamsConfig.pauseWhenDeActivated)
			{
				if(__crocoEngine) __crocoEngine.start();
				if(__starling) __starling.start();
			}
		...
		protected function updateStarlingViewPort():void
		{
			...
			const globalParamsConfig:Object = AppConfig.globalParamsConfig;
			var designWidth:int = globalParamsConfig.designWidth;
			var desighHeight:int = globalParamsConfig.designHeight;
