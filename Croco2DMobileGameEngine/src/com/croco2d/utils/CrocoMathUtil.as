package com.croco2d.utils
{
	...
	public final class CrocoMathUtil
	{
														  ...
														  result:Point = null):Point
		{
			...
			return CrocoMathUtil.caculateIntersectionPoint(px, py, perpendicularLinek, lineP0x, lineP0y, linek);
		}
		...
		public static function matrixTransformAABB(rect:Rectangle, matrix:Matrix, result:Rectangle):Rectangle
		{
			...
			//x0, y0
			MatrixUtil.transformCoords(matrix, x0, y0, helperFlashPoint);
			var minX:Number = helperFlashPoint.x;
			var maxX:Number = helperFlashPoint.x;
			var minY:Number = helperFlashPoint.y;
			var maxY:Number = helperFlashPoint.y;
			
			//x1, y0
			MatrixUtil.transformCoords(matrix, x1, y0, helperFlashPoint);
			minX = MathUtil.min(minX, helperFlashPoint.x);
			maxX = MathUtil.max(maxX, helperFlashPoint.x);
			minY = MathUtil.min(minY, helperFlashPoint.y);
			maxY = MathUtil.max(maxY, helperFlashPoint.y);
			
			//x0, y1
			MatrixUtil.transformCoords(matrix, x0, y1, helperFlashPoint);
			minX = MathUtil.min(minX, helperFlashPoint.x);
			maxX = MathUtil.max(maxX, helperFlashPoint.x);
			minY = MathUtil.min(minY, helperFlashPoint.y);
			maxY = MathUtil.max(maxY, helperFlashPoint.y);
			...
			//x1, y1
			MatrixUtil.transformCoords(matrix, x1, y1, helperFlashPoint);
			minX = MathUtil.min(minX, helperFlashPoint.x);
			maxX = MathUtil.max(maxX, helperFlashPoint.x);
			minY = MathUtil.min(minY, helperFlashPoint.y);
			maxY = MathUtil.max(maxY, helperFlashPoint.y);
