...
//		public static const VIDEO_TYPE_EXTENTION:Array = ["flv", "f4v", "f4p", "mp4"];
		...
		{
			Logger.debug(this, "clear", "Purging all assets, emptying queue");
		...
		public function enqueue(...urls):void
		{
			for each (var rawAsset:Object in urls)
			{
				...
				else if (getQualifiedClassName(rawAsset) == "flash.filesystem::File")
				{
					if (!rawAsset["exists"])
					{
						Logger.warn(this, "enqueue", "File or directory not found: '" + rawAsset["url"] + "'");
					}
				...
				{
					Logger.warn(this, "enqueue", "Ignoring unsupported asset type: " + getQualifiedClassName(rawAsset));
				}
			}
		}
		...
		public function enqueueWithName(url:String, name:String = null):String
		{
			...
			Logger.debug(this, "enqueueWithName", "Enqueuing '" + name + "'");
