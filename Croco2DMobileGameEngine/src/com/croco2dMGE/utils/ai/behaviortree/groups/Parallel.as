...
{
	import com.croco2dMGE.utils.ai.behaviortree.base.GroupOfTasks;
	...
	public class Parallel extends GroupOfTasks
	{
		...
		public function Parallel(failIfChildFailed:Boolean, 
								 successIfChildSucceeded:Boolean, 
								 resultIfNoMoreChilden:TaskResult)
		{
			super(failIfChildFailed, successIfChildSucceeded, resultIfNoMoreChilden);
		}
		...
		public override function run(scope:Scope):TaskResult
		{
			...
			for (var i:int = 0; i < childRecords.length(); i++) 
			{
				...
				// run the task.
				if (record.active || resultIfNoMoreChilden === TaskResult.MORE_TIME) 
				{
					...
					if (result === TaskResult.SUCCESS && successIfChildSucceeded) 
					{
					...
					else if (result === TaskResult.FAILURE && failIfChildFailed) 
					{
						terminate();
		...
		{
			var parallel:Parallel = new Parallel(failIfChildFailed, successIfChildSucceeded, resultIfNoMoreChilden);
