package com.croco2d.input.controllers
{
	import com.croco2d.AppConfig;
	import com.croco2d.input.InputController;
	...
	{
		protected var mKeyActions:Array = [];
		
		public function KeyboardController()
		{
			super();
			
			this.name = AppConfig.KEY_KEY_BOARD_CONTROLLER;
		}
		...
		override protected function onInit():void
		{
			...
			onInitDefaultKeyMapping();
			
			//default arrow keys + space bar jump
			GlobalPropertyBag.stage.addEventListener(KeyboardEvent.KEY_DOWN, onStageKeyDownHandler);
			GlobalPropertyBag.stage.addEventListener(KeyboardEvent.KEY_UP, onStageKeyUpHandler);
		}
		
		protected function onInitDefaultKeyMapping():void
		{
			addKeyAction("left", KeyboardController.LEFT);
			addKeyAction("up", KeyboardController.UP);
			addKeyAction("right", KeyboardController.RIGHT);
			addKeyAction("down", KeyboardController.DOWN);
			addKeyAction("jump", KeyboardController.SPACE);
		}
		...
		override public function dispose():void
		{
			...
			GlobalPropertyBag.stage.removeEventListener(KeyboardEvent.KEY_DOWN, onStageKeyDownHandler);
			GlobalPropertyBag.stage.removeEventListener(KeyboardEvent.KEY_UP, onStageKeyUpHandler);
		}
		
		protected function onStageKeyDownHandler(e:KeyboardEvent):void
		{
			if(mKeyActions[e.keyCode])
			{
				for each (var item:Object in mKeyActions[e.keyCode])
				{
					triggerON(item.name, 1, null, (item.channel < 0 ) ? defaultChannel : item.channel);
				}
			}
		}
		
		protected function onStageKeyUpHandler(e:KeyboardEvent):void
		{
			if (mKeyActions[e.keyCode])
			{
				for each (var item:Object in mKeyActions[e.keyCode])
				{
					triggerOFF(item.name, 0, null, (item.channel < 0 ) ? defaultChannel : item.channel);
				}
			}
		}
		...
		{
			if(!mKeyActions[keyCode])
			{
				mKeyActions[keyCode] = new Vector.<Object>();
			}
			else
			{
				var a:Object;
				for each(a in mKeyActions[keyCode])
				{
					if (a.name == actionName && a.channel == channel) return;
				}
			}
			...
			mKeyActions[keyCode].push({name: actionName, channel: channel});
		}
		...
		public function removeActionFromKey(actionName:String, keyCode:uint):void
		{
			if(mKeyActions[keyCode])
			{
				var actions:Vector.<Object> = mKeyActions[keyCode];
				var i:String;
				for(i in actions)
				{
		...
		public function removeAction(actionName:String):void
		{
			...
			var i:String;
			for each(actions in mKeyActions)
			{
				for(i in actions)
				{
		...
		{
			mKeyActions = [];
			input.stopContrllerActions(this);
		}
